    ...
    while True:
        ...
        with grpc_connection(
            ...
            while True:
                ...
                tmp = copy.deepcopy(server_message)
                current_round = tmp.fit_ins.config["rnd"].sint64

                logger.info(','.join(map(str, [current_round, "", "rec_"+field, "start", t1, c1,"",""])))
                logger.info(','.join(map(str, [current_round, "", "rec_"+field, "end", t2, c2,"",""])))
                ...
                field = server_message.WhichOneof("msg")
                logger.info(','.join(map(str, [current_round, "", "send_"+field, "start", t1, c1,"",""])))
                logger.info(','.join(map(str, [current_round, "", "send_"+field, "end", t2, c2,"",""])))
