...
def aggregate(results: List[Tuple[Weights, int]]) -> Weights:
    ...
    # logger.info(','.join(map(str, [0, "aggregation", "start", time.time_ns(), time.process_time_ns(), "", ""])))
    ...
    # logger.info(','.join(map(str, [0, "aggregation", "end", time.time_ns(), time.process_time_ns(),"", ""])))
